  ...
  getToolDefinitions() {
    return [
      ...
      },
      {
        ...
        description:
          'Get LIVE state (HP, AC, conditions with badge values, effects, immunities/weaknesses/resistances, speeds, saves, hazard details) of tokens. This is the tool for anything mechanical; get-token-details only reports placement and appearance on the active scene, read from each token\'s own synthetic actor — correct for unlinked tokens whose conditions differ from the world actor. Identify one token by tokenId, tokenName, or nearest x/y pixel coordinates, or pass all: true for every token. For HAZARDS the response includes a hazard block with the Disable entry, routine, reset, description, stealth DC and hardness. Fields are null when unavailable/not applicable and empty when genuinely empty. Pass detail: \'full\' for a complete statblock readout (strikes, skills, senses, spellcasting, feats, inventory, GM notes) when you need to run the creature rather than just track its HP. Pass selected: true for the token(s) the GM currently has selected on the canvas, or targeted: true for the GM\'s targeted token(s).',
        inputSchema: {
          type: 'object',
          properties: {
